...
var schema = z2.object({
  ...
  paymentMethod: z2.enum(["card", "pix", "paypal"]).default("card"),
...
async function POST(req) {
  ...
  if (session.user.role !== "PATIENT") {
    return NextResponse2.json({ error: "PATIENT_ROLE_REQUIRED" }, { status: 403 });
  }
  ...
  if (parsed.data.serviceId) {
    ...
    if (svc?.isReturnService) {
      ...
      const providerFilter = providerType === "psychoanalyst" ? { psychoanalystId: providerId } : { professionalId: providerId };
