...
export function friendlyStripeCheckoutError(message: string): string {
  ...
  if (m.includes("payment_method_types") || m.includes("payment method type")) {
    return "Forma de pagamento não disponível na assinatura. Tente novamente ou use outra moeda.";
  }
  if (m.includes("no such price")) {
    return "Preço da assinatura não encontrado no Stripe. Verifique se o Price ID está correto no servidor.";
  }
  if (m.includes("stripe_secret_key") || m.includes("api key") || m.includes("invalid api key")) {
    return "Chave do Stripe inválida no servidor. Verifique STRIPE_SECRET_KEY no Railway.";
  }
  ...
  if (m.includes("currency") && m.includes("price")) {
    return "Moeda do preço não confere com a região escolhida. Contate o suporte.";
  }
