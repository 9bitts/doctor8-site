...
export async function requirePharmacistProfessional() {
  const ctx = await requireProfessional();
  if ("error" in ctx) return ctx;
  if (!isPharmacistSpecialty(ctx.professional.specialty)) {
...
export async function listPatientPharmacyCharts(userId: string) {
  return db.patientRecord.findMany({
    where: {
      linkedUserId: userId,
      professional: {
        specialty: { in: ["Pharmacist", "Clinical Pharmacist", "Pharmacy"] },
      },
    },
    select: {
      id: true,
