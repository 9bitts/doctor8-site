...
function PsychologistLoginForm() {
  ...
  useEffect(() => {
    ...
    if (err === "EmailNotVerified") setError("unverified");
    else if (err === "AccountLocked") setError("locked");
    else if (err === "InvalidVerificationLink") setError("invalidLink");
    else if (err === "VerificationFailed") setError("verificationFailed");
    else if (err) setError("invalid");
  }, [searchParams]);
      ...
      <div className="w-full max-w-md">
            ...
            {LANGUAGES.map((l) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition flex items-center gap-1.5 ${
                  lang === l.code ? "bg-violet-500 text-white" : "text-slate-300 hover:text-white hover:bg-white/10"
                }`}
                aria-label={l.label}
              >
                <span>{l.flag}</span>
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-black text-white tracking-tight">
            Doctor<span className="text-violet-400">8</span>
          </h1>
          <p className="text-slate-400 mt-2 text-sm">{t("login.psychologistTagline")}</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                autoComplete="email"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 transition"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 pr-12 text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 transition"
                />
                <button
                  type="button"
            ...
            <button
              ...
              className="w-full bg-violet-500 hover:bg-violet-400 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition flex items-center justify-center gap-2"
            >
          ...
          <div className="border-t border-white/10 mt-6 pt-6 text-center space-y-3">
            ...
            <Link href="/login" className="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-300 transition">
              <ArrowLeft size={12} />
              {t("login.backToMain")}
            </Link>
          </div>
