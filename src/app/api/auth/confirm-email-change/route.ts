...
export async function GET(req: NextRequest) {
  ...
  if (!token) {
    return redirect(req, "/login/paciente?error=InvalidLink");
  }
  ...
  try {
    ...
    if (verificationToken.expires < new Date()) {
      ...
      return redirect(req, "/login/paciente?error=LinkExpired");
    }
    ...
    if (existing && existing.id !== userId) {
      await db.verificationToken.delete({ where: { token } });
      return redirect(req, "/login/paciente?error=EmailTaken");
    }
    ...
    return redirect(req, "/login/paciente?emailChanged=true");
  } catch (error) {
    console.error("[CONFIRM EMAIL CHANGE ERROR]", error);
    return redirect(req, "/login/paciente?error=ChangeFailed");
  }
}
