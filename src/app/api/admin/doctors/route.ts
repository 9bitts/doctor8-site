...
export async function GET(req: NextRequest) {
  ...
  const emissionCounts = userIds.length
    ? await db.professionalEmissionReport.groupBy({
        by: ["professionalUserId"],
        where: { professionalUserId: { in: userIds } },
        _count: { id: true },
      })
    : [];
  const emissionCountMap = new Map(
    emissionCounts.map((row) => [row.professionalUserId, row._count.id]),
  );
  ...
  const doctors = filtered.map((p) => ({
    ...
    emissionReportCount: emissionCountMap.get(p.userId) ?? 0,
    publicUrl:
