...
export async function PUT(
  ...
  const therapist = await db.integrativeTherapistProfile.findUnique({
    where: { id: params.id },
    select: { id: true, verified: true, availability: true },
  });
  if (!therapist) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  const volunteerBlocks = parseAvailabilityJson(therapist.availability).volunteerBlocks ?? [];
  ...
  return NextResponse.json({ integrativeTherapist: updated });
}
