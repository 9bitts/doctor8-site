...
export default function HumanitarianCampaignPage() {
  ...
  useEffect(() => {
    fetch("/api/auth/session")
      ...
      .then(async (s) => {
        if (!s?.user) {
          router.push(`/login/paciente?callbackUrl=${encodeURIComponent(`/humanitarian/${slug}`)}`);
          return;
        }
      ...
      .catch(() => router.push(`/login/paciente?callbackUrl=${encodeURIComponent(`/humanitarian/${slug}`)}`));
