    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">
          Welcome, Dr. {professional.firstName} 👋
        </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: <Calendar className="text-blue-500" size={20} />, label: "Today's appointments", value: completedToday, bg: "bg-blue-50" },
          { icon: <Users className="text-violet-500" size={20} />, label: "Total consultations", value: totalPatients, bg: "bg-violet-50" },
          { icon: <DollarSign className="text-emerald-500" size={20} />, label: "Total earnings", value: new Intl.NumberFormat("en-US", { style: "currency", currency: professional.currency }).format(earningsTotal / 100), bg: "bg-emerald-50" },
          { icon: <Clock className="text-rose-500" size={20} />, label: "Upcoming", value: professional.appointments.length, bg: "bg-rose-50" },
        ].map((s) => (
              ...
              {professional.appointments.map((apt) => (
                <div key={apt.id} className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">{apt.type === "TELECONSULT" ? "🎥 Teleconsultation" : "🏥 In-person"}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs font-semibold text-slate-700">{format(new Date(apt.scheduledAt), "MMM d")}</p>
      ...
      {!professional.verified && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
          <p className="font-semibold text-amber-800 mb-1">Complete your profile to start receiving patients</p>
          <p className="text-sm text-amber-700 mb-3">Add your availability, pricing and credentials to appear in search results.</p>
