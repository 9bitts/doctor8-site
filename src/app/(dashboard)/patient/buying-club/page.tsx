...
export default function BuyingClubClient() {
  const t = useT();
  ...
  const loadClub = useCallback(async (drugId: string) => {
    setClubLoading(true);
    try {
      const res = await fetch(`/api/patient/buying-club?drugCatalogId=${drugId}`);
  ...
  useEffect(() => {
    ...
    (async () => {
      const url = drugId
        ? `/api/patient/buying-club?drugCatalogId=${drugId}`
        : `/api/patient/buying-club?token=${token}`;
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/patient/buying-club/drugs/search?q=${encodeURIComponent(query.trim())}`
        );
        const data = await res.json();
  ...
  async function handleJoin() {
    ...
    setJoining(true);
    try {
      const res = await fetch("/api/patient/buying-club", {
        method: "POST",
  ...
  function shareLink(): string {
    ...
    if (club?.shareToken) {
      return `${base}/patient/buying-club?club=${club.shareToken}`;
    }
    if (selected) {
      return `${base}/patient/buying-club?drug=${selected.id}`;
    }
    return base;
  }
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
          {clubLoading ? (
            ...
            <>
              ...
              {club.exists && club.activeCount > 0 && (
                <button
                  type="button"
                  onClick={handleShare}
