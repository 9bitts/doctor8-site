...
import {
  Globe, Loader2, ExternalLink, Search, RefreshCw, CheckCircle2, AlertTriangle, MinusCircle,
} from "lucide-react";
...
import type { AdminEightUserRow, AdminSsoClientStats, AdminSsoRecentLogin } from "@/lib/admin/admin-sso-users";

type EightSsoStatus = AdminEightUserRow["ssoStatus"];

function statusBadge(status: EightSsoStatus, t: (k: string) => string) {
  const map: Record<EightSsoStatus, string> = {
    ready: "bg-emerald-50 text-emerald-700 border-emerald-200",
    unverified: "bg-amber-50 text-amber-800 border-amber-200",
    unapproved: "bg-orange-50 text-orange-800 border-orange-200",
  ...
  return (
    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${map[status]}`}>
      {t(`admin.eight.ssoStatus.${status}`)}
    </span>
  );
}

export default function AdminEightUsersClient() {
  const { t, lang } = useI18n();
  const locale = localeOf(lang);
  const [users, setUsers] = useState<AdminEightUserRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(`/api/admin/sso/eight?${qs}`);
    if (res.ok) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {t("admin.eight.subtitle").replace("{{count}}", String(total))}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <a
            ...
            className="inline-flex items-center gap-1.5 text-sm font-semibold text-violet-700 border border-violet-200 bg-violet-50 px-3 py-2 rounded-xl hover:bg-violet-100"
          >
            <ExternalLink size={14} /> {t("admin.eight.openApp")}
          </a>
          <button
            type="button"
            onClick={load}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 flex flex-wrap gap-3 items-end">
        ...
        <div>
          ...
          >
            ...
            {["PROFESSIONAL", "PSYCHOANALYST", "INTEGRATIVE_THERAPIST", "ADMIN"].map((r) => (
        ...
        <div>
          ...
          >
            ...
            {(["ready", "unverified", "unapproved", "locked"] as EightSsoStatus[]).map((s) => (
              <option key={s} value={s}>{t(`admin.eight.ssoStatus.${s}`)}</option>
            ))}
          </select>
        </div>
      </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    <span className="text-[11px] font-semibold uppercase bg-violet-50 text-violet-700 px-2 py-0.5 rounded">
                      {user.role}
                    </span>
                    {user.specialty && (
                      <span className="text-xs text-slate-500">{user.specialty}</span>
                    )}
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-slate-500">
                    ...
                    <span>
                      {t("admin.eight.profileVerified")}: {user.profileVerified ? t("admin.sso.yes") : t("admin.sso.no")}
                    </span>
                  </div>
