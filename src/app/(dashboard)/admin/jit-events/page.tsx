...
// Admin panel to create and manage JIT emergency events (e.g. RS floods action).

import { useState, useEffect } from "react";
import {
  AlertTriangle, Plus, X, Loader2, CheckCircle2, AlertCircle,
  Calendar, Users, Power, PowerOff, Tag, Edit2,
} from "lucide-react";
...
export default function AdminJitEventsPage() {
  ...
  async function createEvent() {
    if (!fName.trim()) { setError("Nome do evento é obrigatório."); return; }
    ...
    try {
      ...
      setSuccessMsg(`Evento "${fName}" criado com sucesso!`);
      setShowForm(false);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            ...
            Eventos de Emergência — Plantão JIT
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Crie e gerencie eventos de atendimento em massa (ex: ação do RS).
            Profissionais vinculados ao evento atendem no modo plantão.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-100 py-16 text-center">
          ...
          <p className="font-semibold text-slate-600">Nenhum evento criado ainda</p>
          <p className="text-sm text-slate-400 mt-1">
            Crie um evento para coordenar atendimentos em massa.
          </p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-slate-500">
                    <span className="inline-flex items-center gap-1">
                      <Calendar size={12} /> Início: {formatDate(ev.startAt)}
                    </span>
                    {ev.endAt && (
                      <span className="inline-flex items-center gap-1">
                        <Calendar size={12} /> Fim: {formatDate(ev.endAt)}
                      </span>
                    )}
                    <span className="inline-flex items-center gap-1">
                      <Users size={12} /> {ev.sessionCount} plantões vinculados
                    </span>
                  </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 sticky top-0 bg-white">
              <h2 className="font-bold text-slate-800 flex items-center gap-2">
                <AlertTriangle size={18} className="text-orange-500" /> Novo evento de emergência
              </h2>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="ex.: Ação RS — Enchentes 2024"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Contexto do evento, instruções para profissionais..."
                  rows={3}
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-medium text-slate-700">
                    Forçar atendimento gratuito para todos os profissionais do evento
                  </span>
                </label>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="ex.: Psicologia — pressione Enter"
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {saving
                    ...
                    : <><AlertTriangle size={14} /> Criar evento</>
                  }
                </button>
              </div>
            </div>
