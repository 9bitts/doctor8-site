"use client";
// src/app/(dashboard)/integrative-therapist/settings/availability/page.tsx
...
export default function IntegrativeAvailabilityPage() {
  ...
  async function fetchAvailability() {
    try {
      const res = await fetch("/api/integrative-therapist/availability");
      if (res.ok) {
  ...
  function getDaySlots(schedule: DaySchedule) {
    return schedule.blocks
      .flatMap((b) => generateSlotsInRange(b.startTime, b.endTime, b.slotDuration, b.slotGap))
  ...
  async function handleSave() {
    ...
    try {
      ...
      await fetch("/api/integrative-therapist/availability", {
        method: "PUT",
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="text-right">
          <p className="text-2xl font-bold text-teal-600">{totalWeeklySlots}</p>
        ...
        {schedules.map((schedule) => {
          ...
          return (
            <div
              ...
              className={`p-5 transition-colors ${schedule.enabled ? "" : "opacity-50 bg-slate-50"}`}
            >
              <div className="flex items-center gap-4 flex-wrap">
                <button
                  ...
                  className={`w-11 h-6 rounded-full transition-colors shrink-0 ${schedule.enabled ? "bg-teal-500" : "bg-slate-200"}`}
                >
                  <div
                ...
                <p className="font-semibold text-slate-800 w-24 shrink-0">
                ...
                {schedule.enabled && daySlots.length > 0 && (
                  <p className="text-xs text-teal-600 font-medium ml-auto">
              ...
              {schedule.enabled && (
                <div className="mt-4 space-y-3 pl-15">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/30 bg-white"
                          >
                            {TIMES.map((tm) => (
                  ...
                  {daySlots.length > 0 ? (
                    <div className="mt-2 p-3 bg-teal-50/60 rounded-xl border border-teal-100">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {daySlots.map((slot, i) => (
                          <span
                            ...
                            className="text-xs bg-white border border-teal-200 text-teal-700 px-2 py-1 rounded-lg tabular-nums"
                          >
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="w-full flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-500 text-white font-semibold py-3.5 rounded-xl transition disabled:opacity-50"
      >
        {saving ? (
