...
import { Eye, EyeOff, Loader2, AlertCircle, FlaskConical, ArrowLeft, Search, LogIn } from "lucide-react";
...
export default function LaboratoriosCadastroPage() {
  ...
  const [labType, setLabType] = useState<"BLOOD" | "IMAGING" | "BOTH">("BLOOD");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({});
    try {
      const res = await fetch("/api/auth/register-laboratory", {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-lg mx-auto px-4 py-10">
        <Link href="/laboratorios" className="inline-flex items-center gap-2 text-slate-400 hover:text-white text-sm mb-8">
          <ArrowLeft size={16} /> Voltar
        </Link>

        <RegisterLogo />
        <div className="flex items-center gap-2 text-violet-400 mb-2 mt-6">
          <FlaskConical size={20} />
          <span className="font-semibold">Cadastro de laboratório</span>
        </div>
        <h1 className="text-2xl font-bold mb-2">Grátis — publique seus preços</h1>
        <p className="text-slate-400 text-sm mb-8">
          Cadastre o CNPJ do laboratório e importe sua tabela de exames em seguida.
        </p>

        {step === 1 ? (
          <div className="space-y-4">
            <label className="block">
              <span className="text-sm text-slate-300">CNPJ do laboratório</span>
              <input
                value={cnpj}
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 py-3 rounded-xl font-semibold disabled:opacity-50"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <label className="block">
              <span className="text-sm text-slate-300">Tipo de laboratório</span>
              <select
                value={labType}
                onChange={(e) => setLabType(e.target.value as typeof labType)}
                className="mt-1 w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3"
              >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 py-3 rounded-xl font-bold disabled:opacity-50"
            >
              {loading ? <Loader2 className="animate-spin" size={18} /> : null}
              Criar conta do laboratório
            </button>
        ...
        <p className="text-center text-slate-500 text-sm mt-8">
          Já tem conta?{" "}
          <Link href={LABORATORY_LOGIN} className="text-violet-400 hover:underline inline-flex items-center gap-1">
