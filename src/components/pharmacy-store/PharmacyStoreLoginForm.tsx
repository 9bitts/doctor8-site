...
import { canAccessPharmacyStorePortal } from "@/lib/pharmacy-portal-guards";
import {
  PHARMACY_STORE_HOME,
...
export default function PharmacyStoreLoginForm() {
  const searchParams = useSearchParams();
  const queryCallback = searchParams.get("callbackUrl") || "";
  const { callback: callbackUrl } = resolveClientAuthCallback(queryCallback || PHARMACY_STORE_HOME);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await signIn("credentials", {
        email: trimmedEmail,
        password,
        callbackUrl: callbackUrl || PHARMACY_STORE_HOME,
        redirect: false,
      });
      ...
      if (session?.user?.role) {
        ...
        const destination = safePostLoginUrl(
          session.user.role,
          savedCallback || callbackUrl || PHARMACY_STORE_HOME,
        );
  ...
  return (
    <LoginPageShell accent="emerald">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-slate-400 text-sm">Portal da drogaria — estoque e preços</p>
      </div>
      ...
      <LoginCard>
        <LoginAlerts
          ...
          verifyFrom={PHARMACY_STORE_LOGIN}
