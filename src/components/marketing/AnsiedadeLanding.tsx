...
import {
  ANSIEDADE_LANDING as C,
  ANSIEDADE_WHATSAPP_DEFAULT_MESSAGE,
  ANSIEDADE_WHATSAPP_DISPLAY,
  ansiedadeWhatsAppHref,
} from "@/lib/ansiedade-landing-content";

const waHref = ansiedadeWhatsAppHref(ANSIEDADE_WHATSAPP_DEFAULT_MESSAGE);

export default function AnsiedadeLanding() {
  return (
    <main>
      <section className="relative overflow-hidden bg-slate-950 min-h-[70vh] sm:min-h-[75vh] flex flex-col">
        ...
        <div className="relative flex flex-col flex-1">
          ...
          <h1 className="mt-4 max-w-2xl text-3xl sm:text-5xl font-bold tracking-tight text-white leading-[1.1]">
            {C.hero.title}
            <span className="block mt-2 text-violet-200 text-2xl sm:text-3xl font-semibold">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-violet-700 px-5 py-3 text-sm font-semibold text-white hover:bg-violet-800 transition shadow-lg shadow-violet-500/25 min-h-11"
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-violet-400/40 px-5 py-3 text-sm font-semibold text-violet-100 hover:bg-violet-500/15 transition min-h-11"
            >
          ...
          <p className="mt-6 max-w-lg text-sm text-slate-200 leading-relaxed flex gap-2">
            <Phone size={16} className="shrink-0 mt-0.5 text-violet-200" />
              ...
              <ol className="mt-8 space-y-5">
                {C.howItWorks.steps.map((s) => (
                  <li key={s.step} className="flex gap-4">
                    <span className="text-2xl font-bold text-violet-500 leading-none w-10 shrink-0">
                      {s.step}
                    </span>
                    <div>
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            ...
            <div>
              ...
              <Link
                ...
                className="mt-6 inline-flex items-center gap-1 text-sm font-semibold text-violet-300 hover:text-violet-200"
              >
      ...
      <section className="bg-slate-50 border-b border-slate-200">
        <div className="max-w-3xl mx-auto px-4 py-14 sm:py-16">
          ...
          <p className="mt-6 text-sm text-slate-600">
            Também cuida de depressão ou saúde física?{" "}
            <Link
              href="/depressao"
      ...
      <section className="bg-violet-700">
        <div className="max-w-6xl mx-auto px-4 py-14 sm:py-16">
          ...
          <p className="mt-3 max-w-xl text-violet-100">{C.finalCta.subtitle}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href={C.finalCta.primary.href}
              className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-semibold text-violet-800 hover:bg-violet-50 transition"
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-medium text-violet-100 hover:text-white transition"
            >
              {C.finalCta.browse.label}
            </Link>
          </div>
          <p className="mt-6 text-xs text-violet-200">
            WhatsApp: {ANSIEDADE_WHATSAPP_DISPLAY} · CVV 188 · SAMU 192
          </p>
        </div>
      </section>
    </main>
