...
export default function LaboratoryLoginForm() {
  const searchParams = useSearchParams();
  const queryCallback = searchParams.get("callbackUrl") || "";
  const { callback: callbackUrl } = resolveClientAuthCallback(queryCallback || LABORATORY_HOME);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await signIn("credentials", {
        email: trimmedEmail,
        password,
        callbackUrl: callbackUrl || LABORATORY_HOME,
        redirect: false,
      });
      ...
      if (session?.user?.role) {
        ...
        const destination = safePostLoginUrl(
          session.user.role,
          savedCallback || callbackUrl || LABORATORY_HOME,
        );
    ...
    <LoginPageShell accent="violet">
      ...
      <div className="text-center mb-6">
        <div className="inline-flex items-center gap-2 text-violet-300 mb-2">
          ...
          <span className="font-semibold text-lg">Doctor8 Laboratórios</span>
        </div>
        <p className="text-slate-400 text-sm">Portal do laboratório — exames e preços</p>
      </div>
      ...
      <LoginCard>
        ...
        <div className="border-t border-white/10 mt-6 pt-6 text-center space-y-3">
          <p className="text-slate-400 text-sm">
            {t("login.noAccount")}{" "}
            <Link href={LABORATORY_REGISTER} className="text-violet-400 hover:text-violet-300 font-medium">
              Cadastrar laboratório
            </Link>
          </p>
          <Link href="/laboratorios" className="text-slate-500 text-xs hover:text-slate-300 transition">
            ← Voltar para doctor8.org/laboratorios
          </Link>
        </div>
      </LoginCard>
    </LoginPageShell>
