...
export default function IntakeFormsModule({ chart }: { chart: NurseChart }) {
  const { t } = useI18n();
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/nurse/charts/${chart.id}/intake-forms`);
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-rose-200 bg-rose-50/50 p-5 space-y-3">
        <h3 className="font-semibold text-slate-900 flex items-center gap-2">
          ...
          {t("nurse.intake.sendTitle")}
        </h3>
        <p className="text-sm text-slate-600">{t("nurse.intake.sendDesc")}</p>
        <ul className="text-sm text-slate-700 space-y-1">
          {NURSING_INTAKE_QUESTIONS.slice(0, 5).map((q) => (
        ...
        >
          <Send size={16} />
          {sending ? t("nurse.saving") : t("nurse.intake.send")}
        </button>
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 font-semibold text-slate-900">
          {t("nurse.intake.history")}
        </div>
        {forms.length === 0 ? (
          <p className="text-sm text-slate-500 p-6 text-center">{t("nurse.intake.noForms")}</p>
        ) : (
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      ...
                      <div className="text-xs text-slate-500 mt-0.5">
                        {f.status === "COMPLETED"
                          ? t("nurse.intake.completed")
                          : t("nurse.intake.pending")}
                      </div>
                    </div>
                    {f.status === "COMPLETED" && (
